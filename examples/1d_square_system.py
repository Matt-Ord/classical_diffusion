...
from classical_diffusion.system import PeriodicSquareSystem1D, UnitSystem


def _plot_periodic_system() -> None:
    system = PeriodicSquareSystem1D(
        gamma=4e11,
        temperature=103,
        m=8e-27,
        delta_x=3e-10,
        barrier_energy=3e-16,
        units=UnitSystem(),
    )
    fig, ax = get_fancy_figure()
    _, ax, _ = plot_periodic_square_potential_1d(system, ax=ax)

    fig.savefig("examples/1d_square_system.potential.pdf")
...
def _plot_1d_periodic_isf() -> None:  # ruff:ignore[too-many-locals]
    ...
    fig.savefig("./examples/1d_square_system.isf.pdf", dpi=300, bbox_inches="tight")
...
def _plot_effective_mass_isf() -> None:  # ruff:ignore[too-many-locals]
    ...
    result_free = solve_ballistic_ensemble(
        normalized_system,
        TimeSpan(
            t_end=normalized_system.units.time_into(1e-12, units=UnitSystem()),
            n_steps=10_000,
        ),
    ...
    effective_mass_exact = UnitSystem().mass_into(
        get_full_effective_mass_exact_1d_square_periodic(
            system=normalized_system, n_samples=100_000_000_000
        ),
        units=normalized_system.units,
    )
    ...
    ax.legend(handles=[line_0, line_1, line_2, line_3])
    fig.savefig(
        "./examples/1d_square_system.effective_mass.pdf", dpi=300, bbox_inches="tight"
    )
...
def _plot_1d_trajectory() -> None:
    ...
    fig.savefig("./examples/1d_square_system.trajectory.pdf")
